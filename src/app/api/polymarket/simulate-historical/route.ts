...
import { runPolymarketBondHistoricalSimulation } from "@/lib/simulation/polymarket-bond-historical"
...
const postSchema = z.object({
  ...
  maxMarkets: z.number().int().min(1).max(100).optional().default(25),
})
  ...
  try {
    ...
    if (strategy.platform !== "polymarket") {
      return NextResponse.json(
        { error: "Polymarket historical simulation requires a Polymarket strategy" },
        { status: 400 }
      )
    }
    ...
    try {
      const { trades, metrics } = await runPolymarketBondHistoricalSimulation({
