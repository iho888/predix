...
- [ ] T-0011: Fix the 16-month sync gap — DB ends 2024-12-30 but today is 2026-05-08. Drives B-0005.
      ...
      DoD:
        ...
        (c) 🟡 PARTIAL 2026-05-08 — Added observability logging to `src/app/api/cron/sync-polymarket/route.ts` (logs every hit before auth, separate log on 401). User must deploy + check Vercel logs to confirm cron is firing. Project IS Vercel-linked (`.vercel/project.json` → projectId `prj_MelZQ2t0GSriXtkMnLrTioi8kS8Q`, project name `predix`); most likely failure is CRON_SECRET unset in Vercel project env.
